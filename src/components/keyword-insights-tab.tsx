  ...
  return (
    <div className="space-y-4 sm:space-y-6 font-apple p-4 sm:p-6 relative">
      ...
      <div className="border-b border-gray-200 bg-white rounded-t-lg sm:rounded-t-xl overflow-hidden">
        <nav className="-mb-px flex gap-2 p-2 sm:p-4">
          {[
            { 
              id: 'keywords', 
              label: 'Keywords', 
            ...
            <button
              ...
              className={`flex-1 sm:flex-none py-2 px-3 sm:px-4 border-2 font-bold text-sm whitespace-nowrap flex flex-col sm:flex-row items-center gap-2 transition-all duration-200 rounded-xl shadow-sm relative ${
                selectedTab === tab.id
                  ...
                  <div>
                    ...
                    <div className="space-y-1">
                      {keyword.videos.slice(0, 3).map((video, videoIndex) => (
                        <div 
                          key={`${keyword.keyword}-${video.id}-${videoIndex}`} 
                          className={`text-xs sm:text-sm text-gray-600 rounded-lg p-3 border border-gray-200 transition-colors ${
                            videoIndex % 2 !== 0 ? 'bg-white hover:bg-emerald-50' : 'bg-slate-100 hover:bg-emerald-50'
                          }`}
                        >
              ...
              <div key={hashtag.hashtag} className="bg-white border border-gray-200 rounded-lg sm:rounded-xl p-3 sm:p-4 hover:shadow-md transition-shadow">
                ...
                {hashtag.videos.length > 0 && (
                  <div>
                    ...
                    <div className="space-y-1">
                      {hashtag.videos.slice(0, 3).map((video, videoIndex) => (
                        <div 
                          key={`${hashtag.hashtag}-${video.id}-${videoIndex}`} 
                          className={`text-xs sm:text-sm text-gray-600 rounded-lg p-3 border border-gray-200 transition-colors ${
                            videoIndex % 2 !== 0 ? 'bg-white hover:bg-emerald-50' : 'bg-slate-100 hover:bg-emerald-50'
                          }`}
                        >
        ...
        <div className="space-y-4 sm:space-y-6">
          ...
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg sm:rounded-xl p-4 sm:p-6">
            <h4 className="font-semibold text-blue-900 mb-3 sm:mb-4 flex items-center gap-2">
              ...
              <span className="text-md sm:text-base font-bold">AI Insights</span>
            </h4>
            <div className="space-y-2 sm:space-y-3">
              {insights.recommendations.insights.map((insight, index) => (
                <div key={index} className="flex items-start gap-2 sm:gap-3">
                  ...
                  <p className="text-blue-800 text-sm sm:text-base leading-relaxed font-medium">{insight}</p>
                </div>
              ))}
            </div>
          </div>
